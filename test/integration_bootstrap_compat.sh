...
no_compat_override="${tmp_dir}/no-compat-override.yml"
override_file="${tmp_dir}/compat-override.yml"

cat >"${no_compat_override}" <<'YAML'
al_folio:
  compat:
    bootstrap:
      enabled: false
...
bundle exec jekyll build --config "_config.yml,${no_compat_override}" -d "${site_no_compat}" >/dev/null
